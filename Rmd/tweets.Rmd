...
broken_ids <- readr::read_csv("broken_tweets.txt", col_names = FALSE) %>%
  ...
  dplyr::mutate(status_id = format(status_id, scientific = FALSE))
readr::write_csv(tibble::tibble(status_id = c(tweets$status_id, broken_ids$status_id)), col_names = FALSE, path = "../data/2020-05-10-status-ids.csv")
```

## Rehydrate tweets by querying Twitter

```{r}
# need multiple calls to get all tweets rehydrated
tweets_rehydrated <- rtweet::lookup_tweets(statuses = tweets$status_id, parse = TRUE)
```
