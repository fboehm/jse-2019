...
In the sections that follow, we detail our backward design-inspired approach to 
...
We note that nearly all of the skills in Table \@ref(tab:circle-table) are 
transferable in that they apply not merely to the thesis projects, but also to
...
(Table \@ref(tab:summative)) with formative and summative strategies.
...
thinking about the scope of possible projects (Supplemental files).
...
We present below reproducible R code for analyzing data from May 2020. While these
...
We analyzed tweets sent on three consecutive 
days from May 24, 2020 to May 26, 2020. We wanted to see if we could detect a transient change in topics for Memorial Day (May 25, 2020). To minimize the computing requirements, we 
limited our analysis to tweets sent during a five-minute period (12:00pm to 12:05pm Eastern time) every day. However, our methods are also appropriate for much larger data sets.
...
we actually analyzed (Supplemental files).
...
chosen topic. 
The goal for latent Dirichlet allocation is to infer both the distribution over topics and the topics [@blei2003latent]. A topic, in this setting, is a distribution over 
...
As a proof of principle of our event detection strategy, we analyzed tweets before, during, and 
after Memorial Day (May 25, 2020). We fitted latent Dirichlet allocation models for each of three
...
We defined each time period to be a single collection, or corpus, of tweets. 
...
seen on May 24 or May 26. Some topics persist, possibly with distinct word probabilities, 
...
In these discussions, we developed action items to advance research progress and overcome challenges. We summatively assessed student achievement at the end of the academic year. Both
...
With future students, we will use a written rubric to evaluate theses (Table \@ref(tab:summative)). We'll
...
establish their own tweet collections. We also presented details for one analysis strategy. 

Our mentoring in data science aligns with others' calls to reconsider the role of computing in statistics
and data science [@nolan2010computing;@carver2016guidelines]. @hicks2018guide argue for
incorporating three concepts into data science classes: computing, connecting and creating. 
They use the terms "connecting" and "creating" to describe the processes of applying 
quantitative methods to real data and real research questions and of research question 
formulation, respectively. Our tweet analysis projects offer students opportunities in all
...
data, visualizing results, and communicating findings. By considering first student 
research interests and integrating them with our senior thesis learning objectives, 
we successfully guided two undergraduate researchers in data science research with
tweets.
...
We include our R code in supplementary files.
