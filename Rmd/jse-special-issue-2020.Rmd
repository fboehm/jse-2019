...
we choose a discrete distribution over topics. For example, some tweets from Memorial Day may refer to the holiday. This may constitute one topic in the corpus. 
...
analyze tweets before, during, and after Memorial Day 2020 (May 25, 2020). We fitted latent Dirichlet allocation models for each of three
...
One challenge in latent Dirichlet allocation modeling is the choice of the 
number of topics per corpus. To address this, we fitted each corpus to models 

We used several criteria to evaluate latent Dirichlet allocation model fits, with 
emphasis on choosing a small number of topics per corpus.
